-- Neovim on_bytes buffer change listener for Snippets.
--
-- Uses nvim_buf_attach with on_bytes to capture exact byte coordinates
-- for every buffer change.  Events are appended to vim.g._snippets_nvim_changes
-- (cleared by the Python side after consumption).

local M = {}

vim.g._snippets_nvim_changes = {}
...
local function make_callback(token)
  return function(_, _buf, _, start_row, start_col, _byte_offset,
    ...
    local changes = vim.g._snippets_nvim_changes
    table.insert(changes, {
      start_row, start_col,
      old_end_row, old_end_col,
      new_end_row, new_end_col,
    })
    vim.g._snippets_nvim_changes = changes
  end
end
...
function M.reset()     vim.g._snippets_nvim_changes = {} end
